...
5. 删除文档。
6. 复制双链引用：复制文章的[内部引用链接](./article#references)。
7. 新窗口查看：打开一个只读的 Blossom 窗口查看文章信息。
8. 博客中查看：如果是公开文章，则可在博客中查看，前提是你需要[部署博客](./deploy/blog)。
9. 浏览器临时访问：可以生成一个临时链接在浏览器中访问文章，该方式无需部署博客即可查看，每个临时访问链接的有效期为 3 个小时。
10. 导出文章：
    - 将文章导出为 Markdown 格式。
...
11. 复制链接：
    ...
    - 复制临时访问链接：复制临时链接。
12. 博客二维码：打开一个二维码弹窗，可以使用移动端扫码访问文章，必须是公开文章才能生成二维码。
